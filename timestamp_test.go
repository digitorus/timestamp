...
func TestParseRequestFromJSONSuccess(t *testing.T) {
	reqHashAlgo := crypto.SHA256
	req := request{
		Version: 1,
		MessageImprint: messageImprint{
			HashAlgorithm: pkix.AlgorithmIdentifier{
				Algorithm:  getOIDFromHashAlgorithm(reqHashAlgo),
				Parameters: asn1.NullRawValue,
			},
			HashedMessage: []byte("abc123"),
		},
		ReqPolicy: asn1.ObjectIdentifier{1, 3, 6, 1, 5, 5, 7, 1, 3},
	...
	parsed, err := ParseRequestFromJSON(jsonBytes)
	if err != nil {
		t.Errorf("expected nil error: %v", err)
	}
	if parsed == nil {
		t.Errorf("expected parsed request not to be nil")
	}

	if parsed.HashAlgorithm != reqHashAlgo {
		t.Errorf("Request.HashAlgorithm does not match request hash algorithm \nActual: %v, expected: %v", parsed.HashAlgorithm, crypto.SHA256)
	}
	if !bytes.Equal(parsed.HashedMessage, req.MessageImprint.HashedMessage) {
		t.Errorf("Request.HashMessage does not match request.MessageImprint \nActual: %v, expected: %v", parsed.HashedMessage, req.MessageImprint.HashedMessage)
	}
	if parsed.Certificates != req.CertReq {
		t.Errorf("Request.Certificates field does not match request.CertReq \nActual: %v, expected: %v", parsed.HashedMessage, req.MessageImprint.HashedMessage)
	}
	if parsed.Nonce.Cmp(req.Nonce) != 0 {
		t.Errorf("Request.Nonce field does not match req.Nonce \nActual: %v, expected: %v", parsed.Nonce, req.Nonce)
	}
	if !parsed.TSAPolicyOID.Equal(req.ReqPolicy) {
		t.Errorf("Request.TSAPolicyOID field does not match req.ReqPolicy \nActual: %v, expected: %v", parsed.TSAPolicyOID, req.ReqPolicy)
	}
	if len(parsed.Extensions) != len(req.Extensions) {
		t.Errorf("Request.Extensions length does not match req.Extensions length \nActual: %v, expected: %v", len(parsed.Extensions), len(req.Extensions))
	}
}
...
func TestParseRequestFromJSONInvalidReqFormat(t *testing.T) {
	...
	if err == nil {
		t.Errorf("expected non-nil error due to invalid request format")
	}
	if parsed != nil {
